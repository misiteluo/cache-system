...
%% API
-export([start_link/0, get_pid/1, get_pids/1, set/2, set_batch/1, delete/1, delete_batch/1, check_consistency/0, handle_store_termination/1]).
...
% 进行一致性检查
check_consistency() ->
  gen_server:cast(?MODULE, check_consistency).
...
handle_call(Request, From, State) ->
  try
    ...
    error:Reason:Stacktrace ->
      ...
      NewState = do_check_consistency(State),
    ...
    [] ->
      try
        ...
        Error:Reason:Stacktrace ->
          ...
          % 进行一致性检查
          {ok, NewState} = gen_server:call(?MODULE, check_consistency),
          {reply, {error, {set_failed, Reason}}, NewState}
      end
  end;
do_handle_call({set_batch, KeyValueList}, _From, #state{monitors = Monitors} = State) ->
  ...
  % 更新状态：使用 NewMonitors，并执行一致性检查
  NewState = do_check_consistency(State#state{monitors = NewMonitors}),
...
do_handle_cast(check_consistency, State) ->
    {noreply, do_check_consistency(State)};
...
% 一致性检查具体逻辑
do_check_consistency(#state{monitors = Monitors} = State) ->
  ...
  RemovedKeys = ets:foldl(
    fun({Key, Pid}, Acc) ->
      case is_process_alive(Pid) of
        ...
        false ->
          ets:delete(?ETS_TABLE, Key),
          logger:warning("移除了键 ~p 的无效PID映射, pid ~p", [Key, Pid]),
          [Key | Acc]
      end
    end,
    [],
